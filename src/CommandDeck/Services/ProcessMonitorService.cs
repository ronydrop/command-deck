...
public class ProcessMonitorService : IProcessMonitorService, IDisposable
{
    ...
    // Cache to avoid running expensive WMI + netstat queries more than once per 30 seconds
    ...
    {
        // Return cached result if still valid (avoids WMI + netstat on every call)
        ...
        try
        {
            // Double-check inside lock: another concurrent caller may have populated cache while we waited
            if (_cachedProcesses != null && DateTime.UtcNow < _cacheExpiry)
                return _cachedProcesses;
        ...
        var ordered = results.OrderBy(p => p.Name).ThenBy(p => p.Pid).ToList();
        _cachedProcesses = ordered;
